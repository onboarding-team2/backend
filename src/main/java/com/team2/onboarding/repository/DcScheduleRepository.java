...
public interface DcScheduleRepository extends JpaRepository<DcSchedule, Long> {
    ...
    @Query("SELECT s FROM DcSchedule s WHERE s.company.id = :companyId " +
    ...
    List<DcSchedule> searchSchedules(
    ...
    Optional<DcSchedule> findByIdAndCompany_Id(Long id, Long companyId);
}
